...
public class CatmullRom
{
    ...
    public static void GetArg(List<Vector3> points, ref List<Vector3> rusults)
    {
        rusults.Clear();
        ...
        float avg = len / (points.Count - 1);

        len = 0;
        rusults.Add(points[0]);
        for (int i = 1; i < points.Count;)
        {
            ...
            {
                float offset = avg - len;
                len = dis - offset;
                Vector3 center = Vector3.Lerp(points[i - 1], points[i], offset / dis);
                rusults.Add(center);

                while (len > avg)
                {
                    len -= avg;
                    center = Vector3.Lerp(points[i],points[i - 1], len / dis);
                    rusults.Add(center);
                }
                i++;
            }
        }
        rusults.Add(points[points.Count - 1]);
    }

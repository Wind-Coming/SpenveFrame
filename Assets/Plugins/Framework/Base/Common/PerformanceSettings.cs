...
{
	private Dictionary<int, int> qualityResolution = new Dictionary<int, int>();
...
#region Member Func
    void Start()
    {
	    qualityResolution.Add(Quality.LOW_QUALITY, 640);
	    qualityResolution.Add(Quality.MIDDLE_QUALITY, 750);
	    qualityResolution.Add(Quality.HIGH_QUALITY, 1080);

		int quality = GetQualityConfig();
		SetQuality(quality);
		
		MsgSystem.Instance.AddListener<int>("OnGameQualityChanged", OnGameQualityChanged);
    }

    private void OnGameQualityChanged(int quality)
    {
	    SetQuality(quality);
    }
    ...
    {
	    //默认高级
        int quality = PlayerPrefs.GetInt("quality", Quality.HIGH_QUALITY);
//         if (quality == 0)
//         {
// #if UNITY_ANDROID
...
//         }
    ...
    IEnumerator SetScreenResolution(int quality)
    {
			...
			#else
			    //if (Screen.width > qualityResolution[quality])
			    {
				    float division = (float)(Screen.height) / (float)(Screen.width);
				    int width = qualityResolution[quality];
				    int height = (int)(division * width);

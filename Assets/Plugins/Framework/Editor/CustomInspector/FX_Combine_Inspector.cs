...
public class FX_Combine_Inspector : ShaderGUI
{
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        BlendType();
        _Editor.ShaderProperty(_Cull, "面剔除");
    ...
    private void MainTexture()
    {
        ...
        if (_UVAnimation.floatValue == 1)
        {
            ...
            if (_MainTextureLog)
            {
                ...
                GUILayout.Label("- 开启CustonData以后，用CustomData1的X和Y控制方向");
                GUILayout.Label("- 开启 '循环UV动画' 后，输入值控制移动速度,关闭以后控制位移距离");
                GUI.color = Color.white;
    ...
    private void Dissolve()
    {
        ...
        if(_UseDissolve.floatValue == 1)
        {
            ...
            {              
                GUI.color = _LogColor;  
                GUILayout.Label("- 贴图的颜色属性用于控制溶解边缘颜色");
                GUILayout.Label("- XY控制缩放，ZW控制UV移动速度");
                GUILayout.Label("- 贴图只使用了r通道，不支持带透明的贴图，彩色贴图只识别r通道");
                GUILayout.Label("- 开启CustomData以后，用CustomData1的Z控制溶解幅度");
                GUILayout.Label("- 边缘软硬同时控制边缘硬度和边缘宽度，数值越小边缘越硬");
                GUI.color = Color.white;
                EditorGUILayout.Space();
            }
    ...
    {
        GUI.color = _TitleColor;
        _Editor.ShaderProperty(_UseRim, "边缘光");
        GUI.color = Color.white;
    ...
    private void BlendType()
    {
        ...
        _Blend = (BlendMode)EditorGUILayout.Popup("混合模式", blenVar, Enum.GetNames(typeof(BlendMode)));
    ...
    private void CustomData()
    {
        ...
        if (_UseCustomdata.floatValue == 1)
        {
            ...
            {   
                ...
                GUILayout.Label("- 2，在 ‘custom vertex stream’ 的列表中添加 ‘custom1.xyz’");
                GUILayout.Label("- 3，打开粒子的 ‘Custom Data’选项");
                GUILayout.Label("- 4，将 ‘Custom1’ 的mode设置为Vector，Number of Componets设置为3");
                GUILayout.Label("- 5，使用Custom1的X和Y来控制主帖图UV流动，具体说明见 ‘开启自定义UV - 说明’");
                GUILayout.Label("- 6，使用Custom1的Z来控制溶解率，具体说明见 ‘溶解 - 说明’");
                EditorGUIUtility.labelWidth = 0f;
                GUI.color = Color.white;
                EditorGUILayout.Space();
            }

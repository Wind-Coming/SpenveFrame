...
namespace SBS
{
    static public class SpriteSheetCombiner
    {
        ...
        [MenuItem("Assets/工具/Combine Sprites", false, 1)]
        ...
        private static void GetRect(List<Texture2D> textures, int padding)
        {
            if(textures.Count < 2)
                return;
            
            List<Texture2D> listH = new List<Texture2D>();
            List<Texture2D> listV = new List<Texture2D>();

            listH.AddRange(textures);
            listV.AddRange(textures);

            listH.Sort((x, y) => y.width.CompareTo(x.width));
            listV.Sort((x, y) => y.height.CompareTo(x.height));


        }

        private static Texture2D GetBigger(List<Texture2D> listH, List<Texture2D> listV)
        {
            if(listH[0].width >= listV[0].height)
            {
                return listH[0];
            }
            else

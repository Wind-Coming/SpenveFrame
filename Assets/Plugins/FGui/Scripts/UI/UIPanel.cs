...
namespace FairyGUI
{
    ...
    public class UIPanel : MonoBehaviour, EMRenderTarget
    {
        ...
        //预览用
        public string animName = "t0";
        
        //预览用
        static object LoadResource(string name, string extension, System.Type type, out DestroyMethod destroyMethod)
        {
            destroyMethod = DestroyMethod.Unload;
            if (name.LastIndexOf("!a") > 0)
            {
                //此处判断是fgui的材质压缩时，可以把透明通道单独拿出来，但我们项目不需要这个功能，故跳过加载，避免底层加载报错
                return default;
            }
            ...
            if (Application.isPlaying)
            {
                if (this.container == null)
                {
                    ...
                    if (!string.IsNullOrEmpty(packagePath) && UIPackage.GetByName(packageName) == null)
                        UIPackage.AddPackage(packagePath, LoadResource);
                }
            }
            else
            {
        ...
        void Update()
        {
            ...
            {
                Transition trans = ui.GetTransition(animName);
                if (trans != null)
                {
                    trans.Play();
                }
                ...
                {
                    Debug.LogError("没有找到动画(UIPanel面板上设置):" + animName);
                }
            }
#endif
        }
